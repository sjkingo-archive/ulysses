...
void *alloc(unsigned int size, unsigned char page_align, heap_t *heap)
{
    ...
    if (iterator == -1) // If we didn't find a suitable hole
    {
        ...
        {
            unsigned int tmp = (unsigned int)lookup_oarray(iterator, &heap->index);
            if (tmp > value)
            {
                value = tmp;
                idx = iterator;
        ...
        else
        {
            ...
            header_t *header = lookup_oarray(idx, &heap->index);
            header->size += new_length - old_length;
            // Rewrite the footer.
    ...
    header_t *orig_hole_header = (header_t *)lookup_oarray(iterator, &heap->index);
    ...
    else
    {
        ...
        remove_oarray(iterator, &heap->index);
    }
...
void free(void *p, heap_t *heap)
{
        ...
        test_header->is_hole)
    {
        ...
        while ( (iterator < heap->index.size) &&
                (lookup_oarray(iterator, &heap->index) != (void*)test_header) )
            iterator++;
